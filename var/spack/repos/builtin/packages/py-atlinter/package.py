...
class PyAtlinter(PythonPackage):
    ...
    depends_on("mxnet+cuda+cudnn+nccl", when="@0.1.1:+cuda", type="run")
    depends_on("mxnet~cuda~cudnn~nccl", when="@0.1.1:~cuda", type="run")
    depends_on("py-atlannot", when="@0.2.2:", type="run")
    depends_on("py-atldld@0.2.2", type="run")
    depends_on("py-numpy", type="run")
    depends_on("py-pillow", type="run")
    depends_on("py-pytorch-fid", type="run")
    depends_on("py-pynrrd", when="@0.2.2:", type="run")
    depends_on("py-pyyaml", when="@0.1.1:", type="run")
    depends_on("py-requests", type="run")
    depends_on("py-scikit-image", when="@0.2.2:", type="run")
    depends_on("py-scipy", when="@0.1.1:", type="run")
