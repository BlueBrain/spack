...
class Neuron(Package):
    ...
    variant(
        "binary",
        default=False,
        description="Create special as a binary instead of shell script",
    )
    variant(
        "coreneuron",
        default=True,
        description="Patch hh.mod for CoreNEURON compatibility",
    )
    variant(
        "cross-compile",
        default=False,
    ...
    variant(
        "multisend",
        default=True,
        description="Enable multi-send spike exchange",
    )
    ...
    variant(
        "rx3d",
        default=False,
        description="Enable cython translated 3-d rxd. Depends on pysetup",
    )
    ...
    # Readline became incompatible with Mac so we use neuron internal
    # readline.  HOWEVER, with the internal version there is a bug which
    # makes Vector.as_numpy() not work!
    ...
    _default_options = ["--without-iv", "--without-x"]
    _specs_to_options = {
        "+cross-compile": [
            "cross_compiling=yes",
            "--without-readline",
    ...
    def filter_compilers(self):
        ...
        nrniv_makefile = join_path(self.prefix, self.archdir, "./bin/nrniv_makefile")
        ...
        # hpe-mpi requires linking to libmpi++ and hence needs to use cxx
        # wrapper
        if self.spec.satisfies("+mpi"):
