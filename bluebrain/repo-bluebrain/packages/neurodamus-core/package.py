...
class NeurodamusCore(SimModel):
    ...
    variant('report', default=True, description="Enable SONATA reporting")
    ...
    def prepare(self):
        filter_file(
            r"UNKNOWN_CORE_VERSION", r"%s" % self.spec.version, join_path("hoc", "defvar.hoc")
        )
        try:
            ...
            filter_file(
                r"UNKNOWN_CORE_HASH", r"'%s'" % commit_hash[:8], join_path("hoc", "defvar.hoc")
            )
